...
STD_ReturnType GPIO_GetPinValue(uint8 Copy_u8Port, uint8 Copy_u8Pin, uint8 *Copy_pu8Value){
    ...
    switch(Copy_u8Port){
        case GPIO_PORTA:
            *Copy_pu8Value = READ_BIT(GPIO_PINA_REG, Copy_u8Pin);
            break;

        case GPIO_PORTB:
            *Copy_pu8Value = READ_BIT(GPIO_PINB_REG, Copy_u8Pin);
            break;

        case GPIO_PORTC:
            *Copy_pu8Value = READ_BIT(GPIO_PINC_REG, Copy_u8Pin);
            break;

        case GPIO_PORTD:
            *Copy_pu8Value = READ_BIT(GPIO_PIND_REG, Copy_u8Pin);
            break;
...
STD_ReturnType GPIO_SetPortDirection(uint8 Copy_u8Port, uint8 Copy_u8Direction){
    if (Copy_u8Port > GPIO_PORTD || (Copy_u8Direction != GPIO_INPUT && Copy_u8Direction != GPIO_OUTPUT)){
        return E_NOK;
    }

    uint8 Local_u8DdrVal = (Copy_u8Direction == GPIO_OUTPUT) ? 0xFF : 0x00;

    switch (Copy_u8Port) {
        case GPIO_PORTA:
            GPIO_DDRA_REG = Local_u8DdrVal;
            break;

        case GPIO_PORTB:
            GPIO_DDRB_REG = Local_u8DdrVal;
            break;

        case GPIO_PORTC:
            GPIO_DDRC_REG = Local_u8DdrVal;
            break;

        case GPIO_PORTD:
            GPIO_DDRD_REG = Local_u8DdrVal;
            break;
...
STD_ReturnType GPIO_GetPortValue(uint8 Copy_u8Port, uint8 *Copy_u8Value){
    if (Copy_u8Port > GPIO_PORTD || Copy_u8Value == NULL){
        return E_NOK;
    }

    switch (Copy_u8Port) {
        case GPIO_PORTA:
            *Copy_u8Value = GPIO_PINA_REG;
            break;

        case GPIO_PORTB:
            *Copy_u8Value = GPIO_PINB_REG;
            break;

        case GPIO_PORTC:
            *Copy_u8Value = GPIO_PINC_REG;
            break;

        case GPIO_PORTD:
            *Copy_u8Value = GPIO_PIND_REG;
            break;
